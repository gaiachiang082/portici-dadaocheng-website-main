...
export function MagazineSection() {
  return (
    <section
      id="rivista"
      className="bg-bg-dark px-6 py-[120px]"
    >
      <div className="mx-auto flex max-w-5xl flex-col items-center gap-12 lg:flex-row lg:items-start lg:gap-16">
        ...
        >
          <div className="group relative mx-auto aspect-[3/4] max-w-sm overflow-hidden border border-red-ritual transition-[transform,box-shadow] duration-[400ms] hover:scale-[1.02] hover:shadow-[0_0_24px_var(--red-glow)]">
            <Image
              src="/ghost-month/images/cover.png"
        ...
        >
          ...
          <ul className="space-y-2">
            {contents.map((item) => (
              <li
                key={item}
                className="font-body text-base text-ink-secondary"
              >
          ...
          >
            <input
              ...
              className="flex-1 border border-[color-mix(in_srgb,var(--ink-secondary)_50%,transparent)] bg-transparent px-4 py-3 font-body text-base text-ink outline-none transition-colors focus:border-gold-incense"
            />
            <button
              type="submit"
              className="border border-gold-incense px-6 py-3 font-label text-[10px] font-extralight uppercase tracking-[0.2em] text-ink transition-colors hover:bg-gold-dim"
            >
              Iscriviti
            </button>
          </form>
